...
function glossaryRows(items) {
  return items.map((item) => `
    <div class="glossary-item">
      ...
      <p class="glossary-meaning">${esc(item.meaning)}</p>
    </div>
  `).join("\n");
}
...
${section.entries.map((entry) => `
  <div class="formula-item">
    ...
    <p class="formula-item-formula">${formulaToHtml(entry.formula)}</p>
    ${entry.note ? `<p class="formula-item-note">${esc(entry.note)}</p>` : ""}
  </div>
`).join("\n")}
</div>
`).join("\n");
...
<aside class="sidebar">
  ...
  <p class="section-intro">Fixed physical constants every engine's physics uses by default (configurable in the Advanced section).</p>
  <div class="glossary-list">${glossaryRows(GAS_PROPERTY_CONSTANTS)}</div>

  <h2>Default design values</h2>
  <p class="section-intro">Starting values for the efficiencies/design constants above &mdash; each is a real number you can change in the Advanced section, not a fixed physical constant.</p>
...
<p class="subtitle">Generated ${esc(generated)} &middot; every formula this engine's physics actually uses, grouped by component &middot; see the in-app config panels for citations back to the source textbook</p>
...
<p class="subtitle">Generated by ThrustForge — ported from the validated aeropropsim engineering core. Symbols follow this project's own notation (e.g. T04 = stagnation temperature at station 4); see each engine's reference/*.md in the project source for full citations and derivations.</p>
</body>
</html>`;
}
