      ...
      <table className="stage-table">
        <thead>
          <tr>
            ...
            <th title={
              isCompressor
                ? "Nominal per-stage temperature rise = (overall ΔT0 from step 1) / n_stages, except the last stage, which is back-solved to close the target overall pressure ratio exactly (Ref §4.2)."
                : "Nominal per-stage temperature drop = (overall ΔT0 from the shaft power balance) / n_stages, except the last stage, which is back-solved to close the overall expansion exactly (Ref §6.1)."
            }>ΔT0 (K)</th>
            <th title={
              isCompressor
                ? "π_stage = (1 + η_st·ΔT0/T0,in)^(γ_c/(γ_c−1)) (Ref §4.2), using this stage's own inlet T0 in the denominator."
                : "p_out/p_in = [1 − ΔT0/(η_tt·T0,in)]^(γ_h/(γ_h−1)) (Ref §6.1), using this stage's own inlet T0 in the denominator — shown here inverted (1/ratio) as the conventional ≥1 expansion ratio."
