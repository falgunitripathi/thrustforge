 ...
 * reference/scramjet.md §2.2). The combustor-exit Mach
      ...
      {open && (
        <>
          <NumberField
            ...
            max={0.08}
            step={0.001}
            hint="f = fuel-air ratio — kilograms of fuel burned per kilogram of air (ṁ_f/ṁ_a). The scramjet is set by how much fuel you burn, not by a target combustor temperature like the other engines: T03 = (f·η_b·Q_R + Cp_c·T02)/(Cp_h·(1+f)) (§2.2). Adding heat to a supersonic flow slows it down toward Mach 1 — too much fuel for the given M2 would need the flow to drop below Mach 1 inside the combustor ('thermal choking'), which isn't possible here, so the engine won't solve. Lower f or raise M2 if that happens."
          />
