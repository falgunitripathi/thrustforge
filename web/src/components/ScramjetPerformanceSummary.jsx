 ...
 * §2.4 (NPTEL p.279-281). Efficiencies use the source's own f<<1 forms
    ...
    <div className="performance-summary">
      <div className="perf-card">
        <FormulaLabel
          ...
          label="Thrust"
          formula="T = mdot_a·[(1+f)·V4 − V1] (NPTEL p.279-280, §2.4) — the exhaust's momentum (air plus burned fuel, leaving at V4) minus the momentum of the air swallowed at flight speed V1. The nozzle is fully expanded to ambient, so there's no pressure-thrust term."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="T/mdot_a = (1+f)·V4 − V1 (NPTEL p.279-280, §2.4) — thrust per kg/s of air, independent of engine size. Small for a scramjet: V4 is only a bit above the (already enormous) flight speed V1."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          className="perf-label"
          label="TSFC"
          formula="TSFC = Thrust-Specific Fuel Consumption — kg of fuel burned per second for every newton of thrust (lower is better). TSFC = mdot_f/T = f/(T/mdot_a) (NPTEL p.279-280, §2.4)."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="Isp = T/(mdot_f·g) (NPTEL p.281, §2.4) — Isp = specific impulse: seconds of one newton of thrust per newton (weight) of fuel burned, the standard yardstick for comparing air-breathing engines with rockets (higher is better). The source leaves mdot unsubscripted; taken as FUEL flow mdot_f, following the same lecturer's own definition elsewhere in the course (reference/scramjet.md §4, judgment call #2). g = 9.80665 m/s²."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="T03/T02 = (M3²/M2²)·[(1+gamma_c·M2²)/(1+gamma_h·M3²)]²·(1+(gamma_h−1)/2·M3²)/(1+(gamma_c−1)/2·M2²) (NPTEL p.279, §2.2) — M3 = Mach number at the combustor exit, solved from this relation on the supersonic (M3 > 1) branch. Heat addition slows the flow, but in a scramjet it STAYS supersonic — the defining feature of the engine."
        />
        <span className="perf-value">
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="eta_P = 2·V1/(V1 + V4) (NPTEL p.280, §2.4) — how much of the jet's added kinetic energy becomes useful thrust work. The source's own f<<1 form is used: its full form leaves out the fuel's own onboard kinetic energy and gives eta_P > 1 at typical scramjet conditions (reference/scramjet.md §4, judgment call #5)."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="eta_th = (V4² − V1²)/(2·f·eta_b·Q_R) (NPTEL p.280-281, §2.4) — how much of the fuel's released heat becomes jet kinetic energy. The source's own f<<1 form (judgment call #5)."
        />
      ...
      <div className="perf-card">
        <FormulaLabel
          ...
          formula="eta_o = eta_P·eta_th (NPTEL p.281, §2.4) — the fraction of the fuel's heat that ends up as useful thrust work."
        />
